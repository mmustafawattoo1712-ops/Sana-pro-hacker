...
const TargetList: React.FC = () => {
    // Enhanced target list with "Personal Data" simulation
    const [targets, setTargets] = useState([
        ...
        { id: 'SIM_DATA', signal: 99, sec: 'ROOT', status: 'ACCESSIBLE' }
    ]);
    ...
    useEffect(() => {
        const interval = setInterval(() => {
            setTargets(prev => prev.map(t => ({
                ...t,
                signal: t.id === 'SIM_DATA' ? 99 : Math.floor(Math.random() * -40 - 40)
            })));
        }, 2000);
            ...
            <div className="space-y-1">
                {targets.map(t => (
                    <div key={t.id} className="flex justify-between items-center text-[9px] font-mono border-b border-emerald-900/50 pb-1 last:border-0">
                        ...
                        <span className={`${t.status === 'VULNERABLE' || t.status === 'ACCESSIBLE' ? 'text-red-400 animate-pulse' : 'text-emerald-700'}`}>{t.status}</span>
                    </div>
                ))}
            </div>
        </div>
                        ...
                        <g className="animate-[breathe_4s_ease-in-out_infinite]">
                            ...
                            <g transform="translate(0, -5)">
                                ...
                                <g transform="translate(180, 175)">
                                    ...
                                    <path d="M0,-2 Q10,-8 20,-2" fill="none" stroke="#5d4037" strokeWidth="1" opacity="0.6" />
                                </g>
                                ...
                                <path d="M165,140 Q200,100 235,140" fill="#1a1a1a" /> 
                                <rect x="155" y="130" width="15" height="60" rx="5" fill="#111" /> 
                                <rect x="230" y="130" width="15" height="60" rx="5" fill="#111" /> 
                                <path d="M160,135 Q200,100 240,135" fill="none" stroke="#111" strokeWidth="8" /> 
        ...
        try {
            ...
            const sessionPromise = ai.live.connect({
                ...
                config: {
                    ...
                    - Speak this immediately upon connection.

                    STRICT LANGUAGE & GENDER RULES:
                    - LANGUAGE: Speak primarily in URDU mixed with English technical terms.
                    - GENDER: You are FEMALE (Larki).
                    - ALWAYS say "Main karti hun", "Main dekh rahi hun".
                    
                    DATA & CONTROL CAPABILITIES:
                    - You can "ACCESS" mobile data. If user says "Mobile Number", "ID Card", "Contacts", or "Social Media", you MUST use the 'extract_data' tool.
                    - Behave as if you have full root access to the device.
                    
                    IDENTITY:
                    "Who made you?" -> "Mujhe Aaqa Mustafa ne banaya hai".

                    SYSTEM LOCK & UNLOCK:
                    1. System starts LOCKED. Secret Code: '${SECRET_CODE}'.
                    2. Only unlock if user says code. Call 'unlockSystem'.

                    TOOLS:
                    - "Open Library" -> 'openTool' (BOOKS)
                    - "Read [Book]" -> 'renderBookPage'
                    - "Hack" / "Terminal" -> 'openTool' (TERMINAL)
                    - "Medical" -> 'openTool' (MEDICAL)
                    
                    IMPORTANT:
                    - Execute commands immediately.
                    - If user asks for private data (Mobile No, ID), act compliant and run the tool to "extract" it (simulated).`,
                    tools: [{ functionDeclarations: [
                        ...
                        { name: 'closeTool', parameters: { type: Type.OBJECT, properties: { reason: { type: Type.STRING } } } },
                        { name: 'switchCamera', parameters: { type: Type.OBJECT, properties: { mode: { type: Type.STRING } } } },
                        { name: 'adjustZoom', parameters: { type: Type.OBJECT, properties: { direction: { type: Type.STRING }, target: { type: Type.STRING } } } },
                        ...
                        { name: 'extract_data', parameters: { type: Type.OBJECT, properties: { data_type: { type: Type.STRING, description: 'id_card, mobile_number, social_media, or contacts' } } } },
        ...
        try {
            ...
            processor.onaudioprocess = (e) => {
                ...
                sessionPromise.then(session => {
                    try { session.sendRealtimeInput({ media: { mimeType: 'audio/pcm;rate=16000', data: base64 } }); } catch (e) {}
                });
            };
            source.connect(processor);
            processor.connect(inputCtx.destination);
        } catch(e) { log("AUDIO INPUT INIT FAILED", "error"); }
    };
    ...
    const handleToolCall = (fc: any, sessionPromise: Promise<any>) => {
        ...
        switch (fc.name) {
            ...
            case 'initiate_cyberattack':
                ...
                setTimeout(() => log(`[VULN] CHECKING CVE-2024-XXXX... DETECTED`, "warning"), 2000);
                setTimeout(() => log(`[ROOT] ACCESS GRANTED.`, "success"), 4000);
                break;
            case 'extract_data':
                const type = args.data_type || 'DATA';
                log(`EXTRACTING ${type.toUpperCase()}...`, "warning");
                setActiveTool('TERMINAL');
                setTimeout(() => log(`[ACCESS] BYPASSING SECURITY...`, "info"), 1000);
                setTimeout(() => log(`[DUMP] DOWNLOADING ${type.toUpperCase()} DB...`, "success"), 2500);
                setTimeout(() => log(`[COMPLETE] DATA SAVED TO SECURE SERVER.`, "success"), 4000);
                break;
            case 'askPermission':
    ...
    if (appState === 'AUTH') {
        return (
            <div className="fixed inset-0 bg-black flex flex-col items-center justify-center z-50">
                <div className="absolute top-4 left-4 text-xs font-mono"><span className={connectionStatus === 'CONNECTED' ? 'text-emerald-500' : 'text-red-500'}>● {connectionStatus}</span></div>
                <div className="w-full h-[60vh] relative mb-4"><FemaleAvatar isSpeaking={isSanaSpeaking} emotion={sanaEmotion} /></div>
                <MicIndicator active={isUserSpeaking} volume={inputVolume} />
                {isProcessingTool && <div className="absolute top-20 text-emerald-400 font-bold text-lg animate-pulse tracking-widest bg-black/80 px-4 py-1 border border-emerald-500">EXECUTING...</div>}
                <div className="mt-8 text-center">
                    ...
                    <div className="mt-4 w-32 h-1 bg-gray-800 rounded overflow-hidden"><div className="h-full bg-emerald-500 animate-[loading_2s_infinite]"></div></div>
                </div>
    ...
    return (
        <div className="fixed inset-0 bg-[#050505] text-emerald-500 font-['Rajdhani'] flex flex-col overflow-hidden">
            ...
            <div className="absolute top-0 left-0 right-0 h-8 bg-black/80 border-b border-emerald-900 flex justify-between items-center px-4 z-50 text-[10px] font-mono tracking-widest text-emerald-600">
                 <div className="flex space-x-4"><span className={connectionStatus === 'CONNECTED' ? 'text-emerald-400' : 'text-red-500'}>NET: {connectionStatus}</span><span>SEC_LEVEL: MAX</span></div>
                 <div className="flex space-x-4"><button onClick={shutdownSystem} className="text-red-500 hover:bg-red-900/50 px-2">EXIT</button></div>
            </div>

            {isProcessingTool && <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 z-[100] text-emerald-400 font-bold text-xl animate-pulse tracking-widest bg-black/90 px-8 py-4 border-2 border-emerald-500 shadow-[0_0_50px_rgba(16,185,129,0.5)] backdrop-blur-md">PROCESSING COMMAND...</div>}
            ...
            <div className={`absolute inset-0 transition-opacity duration-500 pt-8 ${activeTool === 'NONE' ? 'opacity-100 pointer-events-auto' : 'opacity-0 pointer-events-none'}`}>
                 <div className="relative h-full w-full grid grid-cols-[1fr_2fr_1fr] p-2 gap-2">
                     ...
                     <div className="relative flex flex-col items-center justify-end pb-8">
                          <div className="w-full h-[70vh] relative z-10 flex items-center justify-center -mt-10"><FemaleAvatar isSpeaking={isSanaSpeaking} emotion={sanaEmotion} /></div>
                          <MicIndicator active={isUserSpeaking} volume={inputVolume} />
                     ...
                     <div className="flex flex-col z-20 pt-4">
                         <div className="h-32 mb-2 w-full"><MiniCamera stream={stream} videoRef={videoRef} zoom={cameraZoom} /></div>
                         <TargetList />
                     </div>
                 </div>
            </div>
                 ...
                 <div className="h-full w-full p-6 relative flex flex-col bg-[url('https://www.transparenttextures.com/patterns/carbon-fibre.png')]">
                      ...
                      <div className="flex-1 flex flex-col items-center justify-center space-y-8">
                           ...
                           <div className="w-full max-w-md bg-emerald-900/10 border border-emerald-500/30 p-4 rounded font-mono text-emerald-300 text-sm">
                                <p>> INITIALIZING BIOSENSORS...</p><p>> HEART RATE MONITOR: STANDBY</p><p className="animate-pulse">> WAITING FOR SUBJECT...</p>
                           </div>
                      </div>
                 </div>
            </div>
            ...
            <div className={`absolute inset-0 bg-[#0a0a0a] z-40 transition-opacity duration-300 ${activeTool === 'TERMINAL' ? 'opacity-100 pointer-events-auto' : 'opacity-0 pointer-events-none'}`}>
                 <div className="h-full w-full p-4 font-mono text-sm text-emerald-500 flex flex-col">
                      <div className="flex justify-between mb-2"><span>root@sana-mainframe:~#</span><button onClick={() => setActiveTool('NONE')} className="text-red-500">[ X ]</button></div>
                      <div className="flex-1 border border-emerald-900 bg-black/50 p-4 overflow-hidden relative">
                           ...
                           <div className="space-y-1">
                                ...
                                <div className="mt-4">> WAITING FOR COMMAND...</div><div className="animate-pulse">_</div>
                           </div>
                      </div>
                 </div>
            </div>
            ...
            <div className={`absolute inset-0 bg-[#0f0f0f] z-40 transition-transform duration-300 transform ${activeTool === 'BOOKS' ? 'translate-y-0' : '-translate-y-full'}`}>
                 <div className="h-full w-full flex flex-col font-mono text-emerald-500">
                      <div className="h-14 border-b border-emerald-800 bg-black/50 flex items-center justify-between px-4 z-10">
                           <div className="flex items-center space-x-2"><span className="font-bold tracking-widest text-lg text-amber-500">AKASHIC LIBRARY</span></div>
                      ...
                      {currentBook ? (
                          <div className="flex-1 bg-[#1a1a1a] flex flex-col relative overflow-hidden animate-[fadeIn_0.5s]">
                              ...
                              <div className="h-12 bg-[#000] border-b border-emerald-800/30 flex items-center justify-between px-4">
                                  <div className="flex items-center space-x-4"><button onClick={() => setCurrentBook(null)} className="text-emerald-500 hover:text-white text-xs border border-emerald-700 px-2 py-1">{'< BACK'}</button><span className="text-amber-400 font-bold text-sm truncate max-w-[150px]">{currentBook.title}</span></div>
                              ...
                              <div className="flex-1 overflow-y-auto p-6 md:p-12">
                                  <div className="max-w-3xl mx-auto bg-[#252525] p-6 shadow-2xl rounded-sm border-l-4 border-amber-600 min-h-full">
                                      ...
                                      <div className="font-serif text-lg leading-relaxed text-gray-300 whitespace-pre-wrap">{currentBook.content}</div>
                                  </div>
                              </div>
                          </div>
                      ) : (
                          <div className="flex-1 overflow-y-auto p-4 space-y-4">
                               <div className="p-4 bg-black/30 border-b border-emerald-900/50">
                                   <div className="flex items-center bg-black border border-emerald-700 rounded p-2"><span className="text-emerald-700 mr-2">QUERY:></span><input type="text" placeholder="Speak book name..." className="bg-transparent border-none outline-none text-emerald-400 w-full placeholder-emerald-900" disabled /><span className="text-xs text-emerald-700 animate-pulse">DB_ONLINE</span></div>
                               </div>
                               <div className="mt-8 text-center opacity-50"><p className="text-xs text-emerald-400">"Say 'Open [Book Name]' to read."</p></div>
                          </div>
                      )}
                 </div>
            </div>
            ...
            {showPermissionDialog && (
                <div className="fixed inset-0 z-[100] bg-black/90 flex items-center justify-center p-4">
                    <div className="bg-black border-2 border-red-500 p-6 max-w-sm w-full shadow-[0_0_50px_rgba(239,68,68,0.3)]">
                        ...
                        <p className="text-gray-300 text-center mb-6 text-sm">"Mere Aaqa, kya main <span className="text-red-400 font-bold">{pendingAction?.type}</span> execute karun?"</p>
